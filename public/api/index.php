...
try {
    ...
    $subiektPrinter = SubiektGT::getPrinterInstance($cfg);

//    var_dump($subiektGt);
//    var_dump($subiektPrinter);
//    return;

    //Connect or create SubiektGt Windows process
    $subiektPrinterCom = $subiektPrinter->connectPrinter();
    $subiektGtCom = $subiektGt->connect();
    var_dump($subiektPrinterCom);
    //Processing API request.
    $result = false;

    //Create API class object
    if ($method == 'print') {
        $obj = new $class($subiektGtCom, $subiektPrinterCom, $json_request['data']);
        echo "ok";
    }
    else {
        $obj = new $class($subiektGtCom, $subiektPrinterCom, $json_request['data']);
